...
/datum/admins/proc/view_refs(atom/D in world) //it actually supports datums as well but byond no likey
	...
	var/list/dat = list()
	dat += "<h1>References of \ref[D] - [D]</h1><br><a href='?_src_=vars;reference_tracking=\ref[D]'>\[Refresh\]</a><hr>"
	...
	for(var/ref in backrefs)
		...
		if(istype(backreference))
			dat += "<tr><td><a href='?_src_=vars;Vars=[REF(backreference)]'>[REF(backreference)]</td><td>[backreference.type]</td><td>[backrefs[backreference]]</td><td><a href='?_src_=vars;reference_tracking=\ref[backreference]'>\[Follow\]</a></td></tr>"
		else if(islist(backreference))
			dat += "<tr><td><a href='?_src_=vars;Vars=[REF(backreference)]'>[REF(backreference)]</td><td>list</td><td>[backrefs[backreference]]</td><td><a href='?_src_=vars;reference_tracking=\ref[backreference]'>\[Follow\]</a></td></tr>"
		else
	...
	for(var/ref in frontrefs)
		...
		dat += "<tr><td>[ref]</td><td><a href='?_src_=vars;Vars=[REF(backreference)]'>[REF(backreference)]</a></td><td>[backreference.type]</td><td><a href='?_src_=vars;reference_tracking=\ref[backreference]'>\[Follow\]</a></td></tr>"
	dat += "</table><hr>"
	dat = dat.Join()
